...
class FileHotReload:
    ...
    def find_functions_from_dwarf(
        ...
        for module in self.target.module_iter():
            ...
            for sym_idx in range(module.GetNumSymbols()):
                ...
                end_load_addr: int = end_addr.GetLoadAddress(self.target) if end_addr.IsValid() else 0
        ...
        for module in self.target.module_iter():
            ...
            for sym_idx in range(module.GetNumSymbols()):
                ...
                if (
                    ...
                    or func_name == "main"
                    or func_name.startswith(("__cxx_global_var_init", "_GLOBAL__sub_I_"))
                    or line_num == 0
                ):
                    continue
                ...
                if end_load_addr == 0:
                    end_addr: Any = symbol.GetEndAddress()
                    end_load_addr = end_addr.GetLoadAddress(self.target) if end_addr.IsValid() else 0
    ...
    def compile_file_to_shared_lib(
        ...
        with open(temp_modified_cpp, "w") as f:
            ...
            f.write(f"// \n")
            f.write(f"// This file is generated for hot-reloading. To modify and hot reload again,\n")
            f.write(f"// edit the original file: {source_path.name}\n")
            f.write(f"// \n")
            f.write(f"// NOTE: This file contains ALL code from the original source (including main,\n")
            f.write(f"// globals, etc.) to preserve dependencies. Only the hot-reloaded functions\n")
            f.write(f"// are actually used - they're extracted via dlsym() at runtime.\n")
            f.write(f"// \n\n")
            current_line += 11
            ...
            for func_name in functions:
                ...
                if match:
                    return_type = match.group(1)
                    params = match.group(3)
                    f.write(f"{return_type} {func_name}_hotreload_{content_hash}({params});\n")
                    current_line += 1
            ...
            for func_name in functions:
                ...
                if match:
                    ...
                    f.write(f"    return {func_name}_hotreload_{content_hash}({param_call});\n")
                    f.write("}\n\n")
                    current_line += 4
    ...
    def load_shared_library(self, so_path: Path) -> int:
                ...
                if error_addr:
                    try:
                        ...
                        if not error.Success():
                            error_msg = f"unknown error (failed to read: {error.GetCString()})"
        ...
        # After dlopen, LLDB needs to be notified about the new module
        # First, check if LLDB auto-detected it
        ...
        # If not auto-detected, manually add it
        if not module:
            error = lldb.SBError()
    ...
    def patch_function(self, old_addr: int, old_end_addr: int, new_addr: int, function_name: str) -> bool:
        TRAMPOLINE_SIZE: int = 12  # movabsq (10 bytes) + jmp *%rax (2 bytes)

        func_size: int = old_end_addr - old_addr if old_end_addr > old_addr else TRAMPOLINE_SIZE

        if func_size < TRAMPOLINE_SIZE:
            self.log(f"⚠ {function_name}: Function too small ({func_size} bytes), patching entry only")
            trampoline = (
                bytes([0x48, 0xB8]) + struct.pack("<Q", new_addr) + bytes([0xFF, 0xE0])
            )

            trampoline_hex = ' '.join(f'{b:02x}' for b in trampoline)
            self.log(f"  → WriteMemory(0x{old_addr:x}, [{trampoline_hex}], {len(trampoline)} bytes)")
            self.log(f"     Disassembly: movabsq $0x{new_addr:x}, %rax; jmp *%rax")

            error: Any = lldb.SBError()
            self.process.WriteMemory(old_addr, trampoline, error)
            if not error.Success():
                self.log(f"✗ Failed to write JMP for {function_name}: {error.GetCString()}")
                return False
        else:
            trampoline = (
            ...
            self.log(f"     Disassembly: movabsq $0x{new_addr:x}, %rax; jmp *%rax; nop×{remaining_size}")

            error: Any = lldb.SBError()
            bytes_written = self.process.WriteMemory(old_addr, bytes(patch_buffer), error)
    ...
    def refresh_breakpoints_for_source(
        ...
        self.log(f"  Checking {self.target.GetNumBreakpoints()} breakpoints for refresh...")
    ...
    def hotreload_file(
        ...
        dwarf_functions = [
            f for f in dwarf_functions
            if not f['name'].startswith('__') and
               not f['name'].startswith('_GLOBAL__sub_I_') and
               f['name'] != 'main'
        ]
        ...
        self.log(f"  Found {len(dwarf_functions)} functions: {[f['name'] for f in dwarf_functions]}")
        ...
        # Force LLDB to refresh its module and symbol information
        # This seems to be necessary for breakpoints to work reliably
        # We use the command interpreter to force a refresh
        ...
        for func_info in dwarf_functions:
            ...
            if func_name not in _FUNCTION_INFO:
                self.log(f"  Patching {func_name}: 0x{original_addr:x} → 0x{wrapper_addr:x}")
                _FUNCTION_INFO[func_name] = {"original_addr": original_addr}
            else:
                self.log(f"  Re-patching {func_name}: 0x{original_addr:x} → 0x{wrapper_addr:x}")

            self.patch_function(original_addr, original_end_addr, wrapper_addr, func_name)
        ...
        if success_count > 0:
            ...
            for func_info in dwarf_functions:
                func_name = func_info["name"]
                if func_name in _FUNCTION_INFO and "line_number" in _FUNCTION_INFO[func_name]:
                    patched_info[func_name] = {
                        "line": _FUNCTION_INFO[func_name]["line_number"]
                    }

            # Delete old auto-breakpoints from previous hot reloads
            # We identify our auto-breakpoints by the "hotreload_auto" name tag
            deleted_count = 0
            breakpoints_to_delete = []

            # First, collect IDs of all hotreload_auto breakpoints
            for bp_idx in range(self.target.GetNumBreakpoints()):
                breakpoint: Any = self.target.GetBreakpointAtIndex(bp_idx)
                if not breakpoint.IsValid():
                    continue

                # Check if this breakpoint has the hotreload_auto tag
                if breakpoint.MatchesName("hotreload_auto"):
                    # This is one of our auto-breakpoints - check if it's for the current .so
                    ...
                    for loc_idx in range(breakpoint.GetNumLocations()):
                        ...
                        if addr.IsValid():
                            ...
                            if symbol.IsValid():
                                ...
                                # Check if this symbol belongs to the current hot reload
                                if f"_hotreload_{content_hash}" in symbol_name:
                                    is_current = True
                                    break

                    # Delete if it's not for the current .so
                    if not is_current:
                        breakpoints_to_delete.append(breakpoint.GetID())

            # Delete breakpoints by ID
            for bp_id in breakpoints_to_delete:
                self.target.BreakpointDelete(bp_id)
                deleted_count += 1

            if deleted_count > 0:
                self.log(f"  Deleted {deleted_count} old breakpoint(s) from previous hot reloads")
            ...
            for func_info in dwarf_functions:
                ...
                # Get the mangled name of the hot-reloaded function
                # We need to use the renamed function name with the hash
                content_hash = so_path.stem.split('_')[-1]
                ...
                # Create breakpoint using command interpreter with -K 0 to disable skip-prologue
                # The Python API doesn't expose this, so we use the CLI
                ...
                # Find the breakpoint we just created (it's the last one)
                bp = self.target.GetBreakpointAtIndex(self.target.GetNumBreakpoints() - 1)

                if bp.IsValid():
                    # Tag it so we can identify our auto-breakpoints
                    bp.AddName("hotreload_auto")

                    if bp.GetNumLocations() > 0:
                        auto_bp_count += 1
                        # Get the resolved address for display
                        ...
                        if line_entry.IsValid():
                            ...
                            self.log(f"  Auto-breakpoint: {func_name} at {so_cpp_file.name}:{line_num} (addr 0x{addr:x})")
                        else:
                            self.log(f"  Auto-breakpoint: {func_name} at 0x{addr:x}")
                    else:
                        self.log(f"  ⚠ Breakpoint created but has no locations: {hotreload_func_name}")
                else:
            ...
            self.log("=" * 60)
            if auto_bp_count > 0:
                self.log(f"  ✓ Created {auto_bp_count} auto-breakpoint(s) in hot-reloaded code")
                self.log(f"  Next 'continue' will hit breakpoints in new code!")
            self.log(f"  Manual breakpoints: b {so_cpp_file}:<line>")
            self.log("=" * 60)
